...
export const Dashboard = () => {
  ...
  } = useGetUpcomingAppointmentQuery(userId);
  const {
    data: appointments,
  ...
  const completed = appointments.data.filter(
    (el: { completed: boolean }) => el.completed
  );
  ...
  return (
    <div>
      ...
      <Card style="bg-secondary relative flex">
        <div
          className={`absolute top-0 right-0 w-52 md:w-64 lg:w-96 h-full rounded-tr-xl rounded-br-xl bg-cover ${
            upcomingAppointment.data
              ? "bg-seeYouSoonImage"
              : "bg-noAppointmentImage"
          } `}
        ></div>
        <div className="z-10">
          {upcomingAppointment.data ? (
            ...
            <p className={`text-xl font-semibold text-white`}>
              No upcoming appointment, click{" "}
              <NavLink
                to={"/schedule-appointment"}
                className={"underline text-secondary_light"}
              >
                here
              </NavLink>{" "}
              to create one{" "}
      ...
      <Card style="bg-white">
        <p className={`text-xl font-semibold text-secondary`}>
          {lastCompleted ? "Most Recent Vitals Result" : "No Vitals taken yet"}
        </p>
        {lastCompleted ? (
          <>
      ...
      <Card style="bg-primary">
        <p className={`text-xl font-semibold text-white`}>
          {lastCompleted ? "Most Recent Doctor Note" : "No Recent Doctor Note"}
        </p>
        {lastCompleted ? (
          <>
      ...
      <Card style="bg-white">
        ...
        <section className="text-sm space-y-2 mt-2">
          {news?.data
            ?.slice(0, 4)
            .map((el: { image: string; text: string }, i: number) => (
